...
- Apply modern Python idioms (pyupgrade rules)
- Ensure imports are properly sorted (isort style)

## Ruff Configuration

When generating Python projects, include this configuration in `pyproject.toml`:
```toml
[tool.ruff]
line-length = 88
...
known-first-party = []  # Replace with your project's module name
...
All Python code should be:
1. Formatted with `ruff format .`
2. Linted with `ruff check --fix .`
3. Verified to pass `ruff check .` with no errors
